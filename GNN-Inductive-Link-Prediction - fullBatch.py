...
class JointGNN(nn.Module):
    def __init__(self):
        ...
        self.linear1 = nn.Linear(1433,1433)
        self.linear2 = nn.Linear(1433,2)
    ...
    def forward(self, x_feature, graph, samples, edges):
       ...
       # print("x",x)
        ...
        z_tensor = torch.zeros((len(samples), 1433)).to(device)
...
for num_epoch in range(epochs):
    ...
    val_loss,val_acc,_,_=predict_model(gnn_model,G_val, small_samples, adj_val_corrupted, val_false_non_edges, val_false_edges)
...
_,test_acc,test_micro,test_weighted=predict_model(gnn_model,G_test, small_samples, adj_test_corrupted, test_false_non_edges, test_false_edges)
